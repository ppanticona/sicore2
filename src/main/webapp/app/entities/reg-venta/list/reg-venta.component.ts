...
import { EntityArrayResponseType, RegVentaService } from '../service/reg-venta.service';
...
export class RegVentaComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRegVenta): string => this.regVentaService.getRegVentaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.regVentas = this.refineData(dataFromBody);
  }

  protected refineData(data: IRegVenta[]): IRegVenta[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IRegVenta[] | null): IRegVenta[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.regVentaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
