import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { AutorizacionService } from '../service/autorizacion.service';

import { AutorizacionComponent } from './autorizacion.component';

describe('Autorizacion Management Component', () => {
  let comp: AutorizacionComponent;
  let fixture: ComponentFixture<AutorizacionComponent>;
  let service: AutorizacionService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'autorizacion', component: AutorizacionComponent }]), HttpClientTestingModule],
      declarations: [AutorizacionComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(AutorizacionComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(AutorizacionComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(AutorizacionService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 'ABC' }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.autorizacions?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to autorizacionService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getAutorizacionIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getAutorizacionIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
